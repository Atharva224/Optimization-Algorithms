...
class leastSquaresModel:
    ...
    def residual(self, p: np.array):
        ...
        for k in range(self.N):
            xk = self.xData[:, k].reshape((self.n, 1))
            fk = self.fData[0, k]
            myResidual[k, 0] = self.model.objective(xk) - fk
    ...
    def jacobian(self, p: np.array):
        ...
        for k in range(self.N):
            xk = self.xData[:, k].reshape((self.n, 1))
            myJacobian[k, :] = self.model.parameterGradient(xk).T[0]
...
fk = np.array([[2, 3, 2.54, 4.76]])
myErrorVector = leastSquaresModel(myObjective, xk, fk)
residual = myErrorVector.residual(p0)
jacobian = myErrorVector.jacobian(p0)
