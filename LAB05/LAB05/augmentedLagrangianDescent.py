...
def augmentedLagrangianDescent(f, P, h, x0: np.array, alpha0=0, eps=1.0e-3, delta=1.0e-6, verbose=0):
    ...
    if delta <= 0 or delta >= eps:
        raise TypeError('range of delta is wrong!')
    ...
    while True:
        ...
        if stationarity <= eps and feasibility <= delta:
            break
...
print("xmin:", xmin)          # Expected to be close to [[0], [1]]
print("alphamin:", alphamin)  # Expected to be close to -1
'''